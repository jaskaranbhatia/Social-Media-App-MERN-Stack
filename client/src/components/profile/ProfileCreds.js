...
class ProfileCreds extends Component {
  render() {
    ...
    const expItems = experience.map(exp => (
      <li key={exp._id} className="list-group-itm">
        <h4>{exp.company}</h4>
        <p>
          <Moment format="YYYY/MM/DD">{exp.from}</Moment> - 
          {exp.to === null ? (Date.now()) : (<Moment format="YYYY/MM/DD">{exp.to}</Moment>)}
        </p>
    ...
    const eduItems = education.map(edu => (
      <li key={edu._id} className="list-group-itm">
        <h4>{edu.school}</h4>
        <p>
          <Moment format="YYYY/MM/DD">{edu.from}</Moment> - 
          {edu.to === null ? (Date.now()) : (<Moment format="YYYY/MM/DD">{edu.to}</Moment>)}
        </p>
